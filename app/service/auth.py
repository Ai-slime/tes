...
class Auth:
    ...
    newly_added_numbers = set()  # in-memory set to mark accounts added during this run
    ...
    def add_refresh_token(self, number: int, refresh_token: str, name: str = ""):
        ...
        if existing:
            ...
            # mark as newly added for this run (not persisted)
            self.newly_added_numbers.add(int(number))
    ...
    def remove_refresh_token(self, number: int):
        ...
        # remove from in-memory newly added set as well
        if number in self.newly_added_numbers:
            self.newly_added_numbers.discard(number)
